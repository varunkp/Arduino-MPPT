...
//
//    17/06/12 Code optimised by W Kirby to include an LCD.
//------------------------------------------------------------------------------------------------------
...
#define SOL_AMPS_SCALE 11              // the scaling value for raw adc reading to get solar amps scaled by 60 - (Scale by 54 for MAX4173T, 22 for MAX4173F, 11 for MAX4173H)
...
int ledPin_jc2 = 7;                   // LED connected to JC2. This is the heartbeat LED, handy to have for debugging.
//int ledPin_jc3 = 6;                   // LED connected to JC3. I have commented this pin out as it is not necessary.
//int digPin = 4;                       // pin for button arduino shield, again the button is not stricktly necessary and I need this pin for the LCD.
int count = 0;
...
LiquidCrystal lcd(2, 3, 4, 10, 11, 12); //define pins to use for the LCD. I have used these pins as either they were already free or I have free'd them up
//Connect RS to pin 2, E to pin 3, DB4 to pin4, DB5 to pin 10, DB6 to pin 11 and DB7 to pin 12
...
void setup()                            // run once, when the sketch starts
{
  ...
  lcd.begin(20, 4);                    // set up coulumns and rows for LCD. Using a 20x4 display.
}
//------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------
void print_data_to_LCD(void) {
  
  //lcd.clear();          //Clear the display
  lcd.setCursor(0, 0);  //Print headings on the top row of the LCD
  lcd.print("BATTERY");
  lcd.setCursor(9, 0);
  lcd.print("ARRAY");
  lcd.setCursor(16, 0);
  ...
  //-----------------------------------
  //Print PV Volts to LCD
  //-----------------------------------
  lcd.setCursor(9, 1);
  lcd.print("      ");
  lcd.setCursor(9, 1);
  lcd.print(sol_volts/100,DEC);
  lcd.print(".");
  if ((sol_volts%100) < 10) {
    lcd.print("0");
    lcd.print(sol_volts%100,DEC);
...
void loop()                          // run over and over again
{
  ...
  //print_data_to_LCD();                  //print data to the LCD (display is refreshed each run of the loop. This slows the loop down significantly)
  if ((seconds - prev_seconds) > 0) {  
     prev_seconds = seconds;		// do this stuff once a second
    //read_data();                         //read data from inputs
    //run_charger();
    //print_data();                    //print data
    print_data_to_LCD();              //print data to LCD (display is refreshed once per second. Loop runs faster and the display is more stable)
  }
}
//------------------------------------------------------------------------------------------------------
